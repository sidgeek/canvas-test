...
class CanvasHandler extends BaseHandler {
  ...
  __onMouseDown(e) {
    if (isFreeDraw) {
      this.__onMouseDownForFreeDraw(e);
      return;
    }
  ...
  __onMouseMoveForFreeDraw(e) {
    if (!this._freeDraw) return;

    let pointer = this.getPointer(e);
    this._freeDraw.points.push(pointer);
    this.clean()
     // 重绘点
    this.drawPoint(this.ctx);
  }

  drawPoint(ctx, size = 4) {
    // draw a basic circle instead
    const { points } = this._freeDraw;
    ctx.save()
    ctx.lineWidth = size
    // 条件一
    if (points.length < 6) {
  ...
  __onMouseMove(e) {
    if (isFreeDraw) {
      this.__onMouseMoveForFreeDraw(e);
      return;
    }
